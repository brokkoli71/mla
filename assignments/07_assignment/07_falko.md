...
|---------------------|--------|--------------|-------------------------------------|
| Vector Unit         | V      | nopv         | 1                                   |
| Load Unit A         | A      | nopa         | 0                                   |
| Load Unit B         | B      | nopb         | 0                                   |
| Store Unit          | S      | nops         | 0                                   |
| Scalar/Control Unit | X (XM) | nopxm        | 1                                   |
| Movement Unit       | M (XM) | nopxm        | 0                                   |
...
|--------------------------------------|------|------------------------------|
| `vlda.conv.fp32.bf16 cml0, [p0, #0]` |  V   | load from p0 in cml0 bf16 convert to fp32        |
| `movx r6, #1`                        |  X   | 1 in r6                                          |
| `vldb x1, [p1, #0]`                  |  B   | load vector from p1 into x1                      |
| `vmov bmhl2, bmhh4`                  |  V| vector move                                     |
| `mova r0, #60`                       |  A   | use load unit to move?                           |
| `vadd.f dm0, dm0, dm1, r0`           |  V   | vector add  3 vectors + 1 scalar                 |
| `ret lr`                             |  X   |  return lr (link register)                      |
| `mov p1, p4`                         |  M   |  pointer move                                    |
| `vst.conv.bf16.fp32 cml0, [p2, #0]`  |  S   |  store from cml0 to p0, conv from fp32 to bf 16  |
...
| S    | p / Scalar, Vector, Accum.   | p1, x7, r10, bmh0 |
| X    | Scalar                       | r11               |
