...
// TODO: implement tensor kernel
  ...
  // convert tile2 in1 ->fp32; convert tile2 in0 fp32 -> bfp16;                             load tile5
  vconv.fp32.bf16 cml1, x2; vconv.bfp16ebs8.fp32 ex10, dm3;                                 vlda.conv.fp32.bf16	cml0, [p0], #64; vldb x0, [p4, #0]
  vconv.fp32.bf16 cmh1, x3;                                                                 vlda.conv.fp32.bf16	cmh0, [p0], #64; vldb x1, [p4, #64]; padds [p4], #256
  // transpose in1 tile3
  vshuffle x2, x0, x1, r0
  ...
  // convert tile 3 in1 ->fp32; convert tile3 in0 ->bfp16;                                  load tile6
  vconv.fp32.bf16 cml1, x2; vconv.bfp16ebs8.fp32 ex10, dm4;                                 vlda.conv.fp32.bf16	cml0, [p0], #64; vldb x0, [p4, #0]
  vconv.fp32.bf16 cmh1, x3;                                                                 vlda.conv.fp32.bf16	cmh0, [p0], #64; vldb x1, [p4, #64]; padds [p4], #256  
  // transpose in1 tile4
  vshuffle x2, x0, x1, r0
  //                     ; convert tile3 in1 ->bfp16;    ; matrix multiplication tile 2
  vshuffle x3, x0, x1, r1; vconv.bfp16ebs8.fp32 ex11, dm1; vmac.f dm2, dm2, ex10, ex11, r3
  // convert tile4 in1 ->fp32; convert tile4 in0 -> bfp16                                   load tile7
  vconv.fp32.bf16 cml1, x2; vconv.bfp16ebs8.fp32 ex10, dm0;                                 vlda.conv.fp32.bf16	cml0, [p0], #64; vldb x0, [p4, #0]
  vconv.fp32.bf16 cmh1, x3;                                                                 vlda.conv.fp32.bf16	cmh4, [p0], #64; vldb x1, [p4, #64]; padds [p4], #256  
  // transpose in1 tile5
  vshuffle x2, x0, x1, r0
  //                     ; convert tile4 in1 -> bfp16    ; matrix mul tile3
  vshuffle x2, x0, x1, r1; vconv.bfp16ebs8.fp32 ex11, dm1; vmac.f dm2, dm2, ex10, ex11, r3
  // convert in1 tile 5 ->fp32; ; convert tile5 in0 -> bfp16                                load tile8
  vconv.fp32.bf16 cml1, x2; vconv.bfp16ebs8.fp32 ex10, dm0;                                 vlda.conv.fp32.bf16	cml0, [p0], #64; vldb x0, [p4, #0]
  vconv.fp32.bf16 cmh1, x3;                                                                 vlda.conv.fp32.bf16	cmh0, [p0], #64; vldb x1, [p4, #64]; padds [p4], #256  
  // transpose in1 tile6
  vshuffle x2, x0, x1, r0
  ...
  vshuffle x2, x0, x1, r1; vconv.bfp16ebs8.fp32 ex11, dm1; vmac.f dm2, dm2, ex10, ex11, r3
  // convert in1 tile 6 ->fp32; ; convert tile6 in0 -> bfp16
  vconv.fp32.bf16 cml1, x2; vconv.bfp16ebs8.fp32 ex10, dm0
  vconv.fp32.bf16 cmh1, x3
  // transpose in1 tile7
  vshuffle x2, x0, x1, r0
  //                     ; convert tile6 in1 -> bfp16    ; matrix mul tile5;
  vshuffle x2, x0, x1, r1; vconv.bfp16ebs8.fp32 ex11, dm1; vmac.f dm2, dm2, ex10, ex11, r3
  // convert in1 tile 7 ->fp32; ; convert tile7 in0 -> bfp16
  vconv.fp32.bf16 cml1, x2; vconv.bfp16ebs8.fp32 ex10, dm0
  vconv.fp32.bf16 cmh1, x3
  // transpose in1 tile8
  vshuffle x2, x0, x1, r0
  ...
  vshuffle x2, x0, x1, r1; vconv.bfp16ebs8.fp32 ex11, dm1; vmac.f dm2, dm2, ex10, ex11, r3
    // convert in1 tile 8 ->fp32; ; convert tile8 in0 -> bfp16
  vconv.fp32.bf16 cml1, x2; vconv.bfp16ebs8.fp32 ex10, dm0
  vconv.fp32.bf16 cmh1, x3
  // transpose in1 tile9
  vshuffle x2, x0, x1, r0
