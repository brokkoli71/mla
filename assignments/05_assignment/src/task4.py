...
def task_a_and_b():
    ...
    # A swizzle block loads m_l2 column-tiles of A and n_l2 row-tiles of B, each of size
    # k_outer * m_prim * k_prim (resp. n_prim) values -> total values scale as (m_l2 + n_l2),
    # not m_l2 * n_l2. Solve m_l2 + n_l2 <= budget and split it symmetrically.
    k_outer = k // k_prim
    values_per_l2_cache = L2_cache_size // data_type_size_fp16
    values_per_l2_unit = k_outer * m_prim * k_prim
    max_l2_sum = values_per_l2_cache // values_per_l2_unit
    print(f"Max m_l2 + n_l2: {max_l2_sum}")
    m_l2 = n_l2 = power_of_two_contained_in(max_l2_sum / 2)
...
def plot_results(tflops_optimized_6d, tflops_optimized_3d, tflops_baseline):
    import matplotlib.pyplot as plt
    labels = ['Optimized (6D)', 'Optimized (3D)', 'Baseline']
    tflops = [tflops_optimized_6d, tflops_optimized_3d, tflops_baseline]
    plt.bar(labels, tflops, color=['blue', 'green', 'orange'])
    plt.ylabel('TFLOPS')
    plt.title('TFLOPS: 6D-tensor swizzle vs 3D-tensor swizzle vs Baseline')
