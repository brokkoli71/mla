...
@ct.kernel
def contraction(A, B, C, n2: ct.Constant[int], m1: ct.Constant[int], n1: ct.Constant[int], m0: ct.Constant[int], l: ct.Constant[int], k: ct.Constant[int], n0: ct.Constant[int]):
    m3_i = ct.bid(0)
    m2_i = ct.bid(1)
    ...
    for l_i in range(0,l,l_t):
        ...
        B_ = ct.load(
            B, 
            index=(m3_i, m2_i, n2_i, m1_i, n1_i, 0, l_i, 0, 0), 
            shape=(1,1,1,1,1,1,l_t,k,n0), 
            padding_mode=ct.PaddingMode.ZERO
        )
        B_ = ct.reshape(B_, (k*l_t, n0))
        acc += ct.matmul(A_, B_)

    acc = ct.astype(acc, ct.float16)
    acc = ct.reshape(acc, (1, 1, 1, 1, 1, m0, 1, 1, n0))
...
if __name__ == "__main__":
    ...
    opti.permute_dims([0, 1, 2, 3, 4, 6, 5, 8, 7])
    ...
    assert torch.allclose(C_final, expected, atol=3e-0), "The result is incorrect!"
    print("The result is correct!")

    plot_tensor(
        C_final.to('cpu'),
        path=file_dir / 'results' / 'task_2_3_4_try7_torch_16.png',
