...
@ct.kernel
def contraction(A, B, C, m1: ct.Constant[int], n1: ct.Constant[int], l: ct.Constant[int], k: ct.Constant[int], m0: ct.Constant[int], n0: ct.Constant[int]):
    m3_i = ct.bid(0)
    m2_i = ct.bid(1)
    ...
    acc = ct.reshape(acc, (1,1,1,1,1,1,1,m0,n0))
...
if __name__ == "__main__":
    ...
    print(opti.make_executable())
    opti.permute_dims([0, 1, 3 , 2, 4, 5, 8, 6, 7])
    ...
    ct.launch(
        ...
        grid, 
        contraction, 
        (A_opt, B_opt, C_opt, opti.config.dim_sizes[3], opti.config.dim_sizes[4], opti.config.dim_sizes[5], opti.config.dim_sizes[6], opti.config.dim_sizes[7], opti.config.dim_sizes[8])
    )
    ...
    plot_tensor(
        C_final.to('cpu'),
        path=file_dir / 'results' / 'try10_torch_16.png',
